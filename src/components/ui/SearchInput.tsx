...
export function SearchInput({ value, onChange, placeholder, atajo = true, className = '', autoFocus }: Props) {
  ...
  return (
    <div className={`relative flex-1 min-w-0 ${className}`}>
      ...
      <AnimatePresence>
        {value && (
          /* `y: '-50%'` va en la animación y no como clase `-translate-y-1/2`:
             framer-motion reescribe `transform` al animar la escala y borraría
             el centrado vertical, dejando la X por debajo del campo. */
          <motion.button
